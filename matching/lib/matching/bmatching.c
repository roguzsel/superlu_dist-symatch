...
void bmatching(int n,int *ver,int *edges,int *s,double *ws,int *s2, double *ws2,double *weight, int *used,int *match) {
...
// Check if current can become the first suitor of partner
  ...
  int jj;
  for(jj=1;jj<10;jj++) {
    printf("Vertex %d has suitors %d and %d of weights %3.1f and %3.1f \n",jj,s[jj],s2[jj],ws[jj],ws2[jj]);
  }
