...
//  int in[l1];
  ...
  printf("Starting allocation of memory of in \n");
  w_old = 0.0;
  w_new = weight1[0];
  ...
  printf("Done allocating space for in\n");
  in[0] = true;   // The 0'th edge is in the solution

  printf("Starting to go through path of length %d \n",l1);

  printf("Setting match[] value of first vertex %d \n",path1[0]);

  match[path1[0]] = 0;

  for(k=1;k<l1;k++) {
    printf("Processing edge %d \n",k);
    match[path1[k]] = 0;
  ...
  printf("Done going through the path \n");

  match[path1[l1]] = 0;

  printf("Starting match-up \n");
...
cycle_dyn_prog(int l1,int *path1,double *weight1,int *match) {
  ...
  in1 = (int *) malloc((l1)* sizeof(int)); 
  if (in1 == NULL) {
    printf("Unable to allocate space for in1 in cycle_dyn_prog \n");
    return(0.0);
  } 
  in2 = (int *) malloc((l1)* sizeof(int)); 
  if (in2 == NULL) {
    printf("Unable to allocate space for in2 in cycle_dyn_prog \n");
    return(0.0);
  } 
