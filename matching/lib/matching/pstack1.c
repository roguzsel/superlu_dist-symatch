// Parallel Suitor algorithm using a stack
//
...
// "stack" is an array of length n+1, used for storing the vertices that must be worked on
...
void pstack1(int n,int *ver,int *edges,int *s,int *stack,double *ws,omp_lock_t *nlocks,double *weight) {
...
// Test that we still beat the old suitor, this might have changed since last time we checked.
      if ((heaviest > ws[partner]) || ((heaviest == ws[partner]) && (i>s[partner]))) {
        ...
        omp_unset_lock(&nlocks[partner]); // Unlocking partner
        stack[--top] = i;            // push i back on the stack, increase stack size 
      }
    }
