...
void MBSuitor(int n,int *ver,int *edges,int *p,double *ws,omp_lock_t *nlocks,double *weight,int *locallist,int *num_match,int *glob_list,int *matched,int *when) {
...
#pragma omp for schedule(static) private(i)
  for(i=1;i<=n;i++) {
    ...
    for(j=ver[i];j<ver[i+1];j++) { // Loop over neighbors of vertex i
      ...
      if (weight[j] < ws[y])                             // If the weight of the current edge is lighter than the current best or
        continue;                                        // if we cannot offer y something better, move on.

      if ((weight[j] == heaviest) && (y < partner))      // When equal weight, give priority to highest index
        continue;

      if ((weight[j] == ws[y]) && (i < suitor[y]))      // When equal weight, give priority to highest index
        continue;
      heaviest = weight[j];  
      partner = y;
    }
