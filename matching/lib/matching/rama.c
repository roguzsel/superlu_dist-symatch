...
void rama(int n,int *ver,int *edges,int *s,double *ws,double *weight,int *p1) {
  void rama_driver();
  double cost_matching();

  double old_cost = cost_matching(n,ver,edges,weight,s);     // Get the cost of the old matching
  printf("Cost before rama is %8.1lf \n",old_cost);

  double mt1 = omp_get_wtime();

  rama_driver(n,ver,edges,s,ws,weight,p1);                  // Roma type post-processing

  double mt2 = omp_get_wtime();
  double timeRama = mt2 - mt1;

  double new_cost = cost_matching(n,ver,edges,weight,s);     // Get the cost of the matching

  printf("************************************************\n");
  printf("*  After rama %8.1lf, improvement %4.2lf %%   *\n", new_cost,(new_cost-old_cost)/old_cost*100.0);
  printf("*  Time rama %8.6lf                         *\n", timeRama);
  printf("************************************************\n");
}

void rama_driver(int n,int *ver,int *edges,int *s,double *ws,double *weight,int *init) {
  int i,j,x;
  void addEdgeR();
  double cost_matching();
...
// Pick a random vertex 
      long int x = random() % (long int) n;  // 0 to n-1
      int v = x + 1;  // 1 to n
